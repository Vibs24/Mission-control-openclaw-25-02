...
import type { Agent, AgentWorkloadSnapshot } from "../../types";
...
interface AgentCardProps {
  ...
  onClick?: () => void;
  compact?: boolean;
  workload?: AgentWorkloadSnapshot;
  proofComplianceRate?: number;
}

export function AgentCard({ agent, selected, onClick, compact, workload, proofComplianceRate }: AgentCardProps) {
  const statusStyle = getAgentStatusStyle(agent.status);
  const openclawAgentId = getOpenClawAgentId(agent.sessionKey);
  const retired = agent.retired === true || agent.routable === false;
    ...
    >
      ...
      <div className="min-w-0 flex-1">
        <div className="flex items-center gap-1">
          ...
          {retired && (
            <span className="text-[9px] px-1.5 py-0.5 rounded bg-gray-200 text-gray-600 uppercase tracking-wide">
              retired
            </span>
          )}
        </div>
        {!compact && (
          <div className="min-w-0">
            ...
            {agent.specialty && <div className="text-[9px] text-gray-400 truncate">{agent.specialty}</div>}
            {workload && (
              <div className="text-[9px] text-gray-500 truncate">
                workload: R{workload.runningCount} Q{workload.queuedCount} S{workload.stuckCount24h}
              </div>
            )}
            {proofComplianceRate !== undefined && (
              <div className="text-[9px] text-gray-500 truncate">
                proof: {Math.round(proofComplianceRate * 100)}%
              </div>
            )}
            {openclawAgentId && (
