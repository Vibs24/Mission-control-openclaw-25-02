...
export const claimNextInboxTaskForChief = mutation({
  ...
  handler: async (ctx, args) => {
    ...
    const chiefIds = Array.from(
      new Set(
        tasks
          .map((t) => t.chiefAgentId)
          .filter((id): id is typeof args.chiefAgentId => Boolean(id))
      )
    );
    const chiefRows = await Promise.all(chiefIds.map((id) => ctx.db.get(id)));
    const chiefMetaById = new Map(
      chiefRows
        .filter(Boolean)
        .map((row) => [
          String(row!._id),
    ...
    const candidate = tasks
      .filter((t) => {
        if (!t.chiefAgentId) return true;
        if (t.chiefAgentId === args.chiefAgentId) return true;
        const meta = chiefMetaById.get(String(t.chiefAgentId));
        if (!meta) return true;
        if (meta.retired) return true;
        if (!meta.routable) return true;
        if (meta.roleKey === "legacy") return true;
        return false;
      })
...
export const setTaskNextCheck = mutation({
  ...
  handler: async (ctx, args) => {
    const task = await ctx.db.get(args.taskId);
    if (!task) {
      return { ok: false, reason: "task_not_found" as const };
    }
